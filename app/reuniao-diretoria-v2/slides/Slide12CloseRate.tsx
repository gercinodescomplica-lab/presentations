...
function CloseRateTable({ title, row, delay = 0 }: { title: string; row: TableData; delay?: number }) {
  ...
  return (
    <motion.div
      ...
      className="flex-1 flex flex-col rounded-lg overflow-hidden border border-[#1E4870] min-h-0"
    >
      {/* Section title */}
      <div className="shrink-0 px-4 py-2.5 bg-[#002B48] text-center">
        <span className="text-white font-semibold text-[15px] tracking-wide">{title}</span>
      </div>

      {/* Column headers */}
      <div className="shrink-0 flex bg-[#1E4870]">
        {COLS.map((col, i) => (
          <div
            key={i}
      ...
      <div className="flex-1 flex bg-[#001F35]/80 border-t border-[#1E4870]">
        {cells.map((cell, i) => {
          const isCR      = i === 9 || i === 10
          ...
          return (
            <div
              key={i}
              className="flex items-center justify-center border-r border-[#1E4870] last:border-r-0 px-1"
              style={{ width: COLS[i].w, flexShrink: 0, flexGrow: isPeriodo ? 1 : 0 }}
            >
              <span
                className={`font-mono text-center leading-snug whitespace-pre-line ${
                  isCR
                    ? 'text-[#FF671D] font-bold text-[28px]'
                    : isValue
                    ? 'text-[#8FA8BD] text-[14px]'
                    : isPeriodo
...
export default function Slide12CloseRate() {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#002B48] text-white overflow-hidden relative">
      ...
      <motion.div
        ...
        className="relative z-10 shrink-0 flex items-center gap-3 px-8 pt-5 pb-3 border-b border-[#1E4870]"
      >
        <div className="p-2 rounded-lg bg-[#FF671D]/10 border border-[#FF671D]/25 shrink-0">
      ...
      <div className="relative z-10 flex-1 min-h-0 flex flex-col gap-4 px-8 py-5">
        ...
        <motion.div
          ...
          className="shrink-0 flex items-center gap-5 text-[#8FA8BD] text-[13px] font-mono"
        >
          <span><span className="text-[#FF671D] font-bold">F#</span> = Nº Finalizadas</span>
          <span><span className="text-[#FF671D] font-bold">F$</span> = Valor Finalizadas</span>
          <span><span className="text-[#FF671D] font-bold">P#</span> = Nº Perdidas</span>
          <span><span className="text-[#FF671D] font-bold">P$</span> = Valor Perdidas</span>
          <span><span className="text-[#FF671D] font-bold">CR#</span> = Close Rate por Oportunidades</span>
          <span><span className="text-[#FF671D] font-bold">CR$</span> = Close Rate por Valores</span>
        </motion.div>
      </div>
    </div>
  )
