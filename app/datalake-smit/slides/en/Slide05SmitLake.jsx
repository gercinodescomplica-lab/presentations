...
const systems = [
  { icon: '📞', name: 'SAC', desc: 'Citizen attendance system — municipal requests and service management', tag: 'Attendance' },
  { icon: '🎓', name: 'Descomplica SP', desc: 'Digital service simplification platform', tag: 'Digital Gov' },
  { icon: '📅', name: 'Agendamento SMIT', desc: 'Municipal scheduling and attendance tracking', tag: 'Operations' },
  { icon: '📋', name: 'Novo 156 – CRM', desc: 'Citizen request management and relationship platform', tag: 'CRM' },
  { icon: '📊', name: 'Analytics Panels', desc: 'Existing BI dashboards and reports', tag: 'Analytics' },
  { icon: '⚙️', name: 'Other Systems', desc: 'Legacy and new SMIT-managed platforms', tag: 'Extended' },
]
      ...
      >
        ...
        <div className="relative z-10 w-[40%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-6 border-r"
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} whileHover={{ scale: 1.015, boxShadow: '0 4px 20px rgba(47,128,255,0.10)', transition: { duration: 0.22, ease: [0.22, 1, 0.36, 1] } }} transition={{ delay: 0.65 }}
            ...
            The SMIT Data Lake is the first deployment of the Paulistano model — integrating key citizen-facing systems under a unified data layer managed jointly by SMIT, PRODAM, and Microsoft.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} whileHover={{ scale: 1.015, y: -4, boxShadow: '0 8px 32px rgba(47,128,255,0.18), 0 0 0 1px rgba(47,128,255,0.12)', transition: { duration: 0.22, ease: [0.22, 1, 0.36, 1] } }} transition={{ delay: 0.85 }}
            ...
            <div className="flex items-center gap-2 pb-2" style={{ borderBottom: `1px solid ${BLUE}15` }}>
              <span className="text-xs font-mono font-semibold tracking-wider" style={{ color: BLUE }}>WHAT THIS ENABLES</span>
            </div>
            {['Cross-system citizen journey view', 'Real-time service demand analytics', 'Predictive capacity planning'].map((item, i) => (
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center px-10 lg:px-12 py-10">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} whileHover={{ scale: 1.015, boxShadow: '0 4px 20px rgba(47,128,255,0.10)', transition: { duration: 0.22, ease: [0.22, 1, 0.36, 1] } }} transition={{ delay: 0.3 }}
            ...
            <span className="text-xs font-mono font-bold tracking-widest" style={{ color: '#7B93C4' }}>
              CONNECTED SYSTEMS · {systems.length} SOURCES
            </span>
          </motion.div>
        ...
        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(47,128,255,0.5)' }}>05 · SMIT DATA LAKE</span>
        </div>
      </motion.div>
    </div>
  )
