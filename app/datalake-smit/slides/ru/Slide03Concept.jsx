...
const properties = [
  { icon: '📦', title: 'Хранит всё', desc: 'Структурированные, полуструктурированные и неструктурированные данные — без предварительного преобразования. Каждая деталь сохраняется.', color: BLUE },
  { icon: '⚡', title: 'Неограниченный масштаб', desc: 'От гигабайт до петабайт — озеро растёт вместе с потребностями без переработки архитектуры.', color: BLUE_DARK },
  { icon: '🔍', title: 'Анализ по запросу', desc: 'Данные готовы для любых запросов — BI, машинного обучения или управленческих отчётов — в любой момент.', color: BLUE },
  { icon: '🔒', title: 'Управление по дизайну', desc: 'Контроль доступа, прослеживаемость и соответствие LGPD встроены с момента приёма данных.', color: BLUE_DARK },
]
    ...
    <div className="w-full h-full flex items-center justify-center p-6 lg:p-10">
      ...
      >
        ...
        <div className="relative z-10 w-[42%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-5 border-r"
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} whileHover={{ scale: 1.015, boxShadow: '0 4px 20px rgba(47,128,255,0.10)', transition: { duration: 0.22, ease: [0.22, 1, 0.36, 1] } }} transition={{ delay: 0.65 }}
            ...
            <p className="text-sm leading-relaxed" style={{ color: '#0D1B3E', fontWeight: 500 }}>
              🌊 Представьте город как речной бассейн.
            </p>
            <p className="text-sm leading-relaxed mt-2" style={{ color: '#3B5794' }}>
              Каждый секретариат — это река, ежедневно генерирующая данные: записи об обслуживании, транзакции, события. <strong>Data Lake</strong> захватывает эти потоки в исходном формате — сохраняя каждую деталь для будущего анализа, не требуя изменений в исходных системах.
            </p>
          </motion.div>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} whileHover={{ scale: 1.015, boxShadow: '0 4px 20px rgba(47,128,255,0.10)', transition: { duration: 0.22, ease: [0.22, 1, 0.36, 1] } }} transition={{ delay: 0.8 }}
            ...
            В модели Paulistano каждый секретариат поддерживает собственное озеро данных — подключённое к общему слою аналитики, охватывающему весь город.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} whileHover={{ scale: 1.015, y: -4, boxShadow: '0 8px 32px rgba(47,128,255,0.18), 0 0 0 1px rgba(47,128,255,0.12)', transition: { duration: 0.22, ease: [0.22, 1, 0.36, 1] } }} transition={{ delay: 1.0 }}
            ...
            <span className="text-xs font-mono tracking-widest" style={{ color: BLUE }}>АКТИВНЫЙ ПИЛОТ · SMIT</span>
          </motion.div>
        </div>
        ...
        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(47,128,255,0.5)' }}>03 · КОНЦЕПЦИЯ</span>
        </div>
      </motion.div>
    </div>
  )
