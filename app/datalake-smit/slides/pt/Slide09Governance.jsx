    ...
    <div className="w-full h-full flex items-center justify-center p-6 lg:p-10">
      ...
      >
          ...
          {committees.map((c, i) => (
            <motion.div key={c.name}
              ...
              <div className="flex flex-col gap-2 mt-auto">
                <div className="text-[10px] font-mono font-bold tracking-widest mb-1" style={{ color: 'rgba(47,128,255,0.5)' }}>MEMBROS</div>
                {c.members.map(m => (
                  <div key={m} className="flex items-center gap-2 px-3 py-2 rounded-xl"
                    ...
                    <span className="text-xs" style={{ color: '#6B84B8' }}>{m}</span>
                  </div>
                ))}
              </div>
            </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} whileHover={{ scale: 1.015, y: -4, boxShadow: '0 8px 32px rgba(47,128,255,0.18), 0 0 0 1px rgba(47,128,255,0.12)', transition: { duration: 0.22, ease: [0.22, 1, 0.36, 1] } }} transition={{ delay: 1.0 }}
          ...
          style={{ borderColor: 'rgba(47,128,255,0.1)', background: 'rgba(47,128,255,0.04)' }}>
          <span className="text-xs font-mono" style={{ color: '#6B84B8' }}>
            Os três comitês reportam a um Grupo Diretor do Data Lake — presidido conjuntamente pela liderança da SMIT e PRODAM
          </span>
        </motion.div>

        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(47,128,255,0.5)' }}>09 · GOVERNANÇA</span>
        </div>
      </motion.div>
    </div>
  )
