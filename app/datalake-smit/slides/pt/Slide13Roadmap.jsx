...
const phases = [
  ...
  { number: '02', title: 'Primeiros Dados', period: 'T4 2026', color: BLUE, status: 'PLANEJADO', items: ['SAC integrado ao data lake', 'Descomplica SP conectado', 'Primeiros dashboards operacionais', 'Validação de qualidade de dados'] },
  { number: '03', title: 'Operação Plena', period: 'T1 2027', color: GREEN, status: 'ROADMAP', items: ['Agendamento SMIT integrado', 'Analytics em tempo real', 'Relatórios automatizados de gestão', 'Auditoria ANPD concluída'] },
  { number: '04', title: 'Inteligência', period: '2027+', color: AMBER, status: 'VISÃO', items: ['Modelos preditivos de demanda', 'Detecção de anomalias em tempo real', 'APIs abertas para parceiros gov', 'IA assistindo gestores SMIT'] },
]
        ...
        <div className="relative z-10 flex items-center justify-between px-12 pt-8 pb-5 border-b"
          ...
          <div>
            ...
            <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} whileHover={{ scale: 1.015, y: -4, boxShadow: '0 8px 32px rgba(47,128,255,0.18), 0 0 0 1px rgba(47,128,255,0.12)', transition: { duration: 0.22, ease: [0.22, 1, 0.36, 1] } }} transition={{ delay: 0.35 }}
              ...
              Da fundação à inteligência SMIT
            </motion.h2>
          </div>
        </div>
        ...
        <div className="relative z-10 flex-1 grid grid-cols-4 gap-0">
          ...
          {phases.map((phase, i) => (
            <motion.div key={phase.number}
              ...
              <div>
                <div className="font-heading font-bold mb-0.5" style={{ fontSize: '1.2rem', color: '#0D1B3E' }}>{phase.title}</div>
