...
const principles = [
  { icon: '🏗️', title: 'Arquitetura Federada', desc: 'Cada secretaria possui um Data Lake dedicado, conectado a uma malha de dados municipal.', color: BLUE },
  { icon: '🔄', title: 'Integração Não Invasiva', desc: 'Os dados são replicados — não migrados. Os sistemas de origem continuam funcionando sem alterações.', color: CYAN },
  { icon: '🔍', title: 'Análises Unificadas', desc: 'Consultas e insights entre secretarias sem centralizar dados brutos em um único ponto.', color: BLUE },
  { icon: '🛡️', title: 'Privacidade by Design', desc: 'Conformidade com a LGPD incorporada em todas as camadas, da ingestão ao controle de acesso.', color: CYAN },
]
      ...
      >
        ...
        <div className="relative z-10 w-[42%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-6 border-r"
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.7 }}
            ...
            Uma cidade.<br />Uma arquitetura de dados.
          </motion.h2>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.65 }}
            ...
            O Data Lake Paulistano é uma rede federada de data lakes por secretaria, cada um gerido pelo próprio órgão — mas conectados a uma camada de inteligência compartilhada que abrange toda a cidade.
          </motion.p>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}
            className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.45)' }}>
            Pense como uma constelação: cada estrela brilha de forma independente, mas juntas iluminam um mapa completo de São Paulo.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }}
            ...
            <span className="text-xs font-mono tracking-widest" style={{ color: CYAN }}>PILOTO ATIVO · SMIT</span>
          </motion.div>
        </div>
        ...
        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(255,255,255,0.3)' }}>03 · CONCEITO</span>
        </div>
      </motion.div>
    </div>
  )
