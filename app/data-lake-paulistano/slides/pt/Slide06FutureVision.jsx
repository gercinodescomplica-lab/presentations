      ...
      >
        ...
        <div className="relative z-10 w-[42%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-6 border-r"
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}
            ...
            Este é o alicerce da experiência de governo único — onde São Paulo age como uma entidade coesa, e não como 30 burocracias separadas.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }}
            className="flex gap-4">
            {[{ v: '30+', l: 'Secretarias' }, { v: '12M', l: 'Cidadãos' }, { v: '1', l: 'Experiência' }].map((s, i) => (
          ...
          <div className="grid grid-cols-4 gap-3">
            {secretarias.map((s, i) => (
              <motion.div key={s.name}
                ...
                style={{ background: s.active ? `${s.color}10` : 'rgba(255,255,255,0.02)', border: `1px solid ${s.active ? s.color + '40' : 'rgba(255,255,255,0.08)'}`, boxShadow: s.active ? `0 0 20px ${s.color}20` : 'none' }}>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.55)' }}>
                  Um cidadão abre o app 156 e a prefeitura já sabe quem ele é, o que precisa e como ajudá-lo — de forma integrada, em todas as 30+ secretarias.
                </p>
              </div>
            </div>
          </motion.div>
        ...
        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(255,255,255,0.3)' }}>06 · VISÃO DE FUTURO</span>
        </div>
      </motion.div>
    </div>
  )
