      ...
      >
        ...
        <div className="relative z-10 flex items-center justify-between px-12 pt-8 pb-4 border-b"
          ...
          <div className="flex flex-col gap-1">
            ...
            <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
              ...
              Федеративные озёра данных — связанный город
            </motion.h2>
          </div>
        ...
        <div className="relative z-10 flex-1 flex items-center justify-center">
          <div className="relative" style={{ width: 600, height: 560 }}>
            ...
            <motion.div
              ...
              style={{
                ...
                border: '1px dashed rgba(47,128,255,0.2)',
              }} />
            ...
            <motion.div
              ...
              style={{
                ...
                top: '50%', left: '50%',
                transform: 'translate(-50%, -50%)',
                background: `linear-gradient(135deg, ${BLUE}22 0%, ${CYAN}15 100%)`,
              ...
              return (
                <motion.div key={agency.name}
                  ...
                  <svg
                    ...
                    style={{
                      width: Math.abs(pos.x) + 10,
                      height: Math.abs(pos.y) + 10,
                      top: pos.y < 0 ? pos.y : -10,
                      left: pos.x < 0 ? pos.x : -10,
                      overflow: 'visible',
                      zIndex: -1,
                    }}>
                    <motion.line
                      x1={pos.x < 0 ? Math.abs(pos.x) + 10 - Math.abs(pos.x) / 2 : 10 + Math.abs(pos.x) / 2}
                      y1={pos.y < 0 ? Math.abs(pos.y) + 10 - Math.abs(pos.y) / 2 : 10 + Math.abs(pos.y) / 2}
                      x2={pos.x < 0 ? 10 : Math.abs(pos.x) + 10}
                      y2={pos.y < 0 ? 10 : Math.abs(pos.y) + 10}
                      stroke={agency.color}
                      strokeWidth="1"
                      strokeOpacity="0.3"
                      strokeDasharray="4 4"
            ...
            <svg className="absolute inset-0 pointer-events-none" width="600" height="560" style={{ zIndex: 1 }}>
              {agencies.map((agency, i) => {
                ...
                const nodeEndX = endX - polarToXY(agency.angle, 52).x
                const nodeEndY = endY - polarToXY(agency.angle, 52).y
                return (
          ...
          <motion.div
            ...
            <span className="text-[10px] font-mono font-bold tracking-widest" style={{ color: 'rgba(255,255,255,0.4)' }}>ЛЕГЕНДА</span>
            {[
              { color: BLUE, label: 'Озеро данных ведомства' },
              { color: CYAN, label: 'Активный пилот' },
              { color: 'rgba(255,255,255,0.2)', label: 'Будущая интеграция' },
            ].map(item => (
        ...
        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(255,255,255,0.3)' }}>04 · АРХИТЕКТУРА</span>
        </div>
      </motion.div>
    </div>
  )
