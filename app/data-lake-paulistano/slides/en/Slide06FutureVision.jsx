      ...
      >
        ...
        <div className="relative z-10 w-[42%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-6 border-r"
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.65 }}
            ...
            As each agency goes live, their data lake joins the Paulistano network. A citizen interacting with SME, SMS, and SMIT leaves one connected trail — enabling city services to anticipate needs, not just react to them.
          </motion.p>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}
            className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.4)' }}>
            This is the foundation of a single-government experience — where São Paulo acts as one coherent entity, not 30 separate bureaucracies.
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }}
            className="flex gap-4">
            {[{ v: '30+', l: 'Agencies' }, { v: '12M', l: 'Citizens' }, { v: '1', l: 'Experience' }].map((s, i) => (
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center px-10 lg:px-12 py-14 gap-6">
          ...
          <div className="grid grid-cols-4 gap-3">
            {secretarias.map((s, i) => (
              <motion.div key={s.name}
                ...
                {s.active && (
                  <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ background: `${s.color}15`, color: s.color }}>ACTIVE</span>
                )}
              </motion.div>
            ))}
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.3 }}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-xs font-mono font-bold tracking-wider mb-1" style={{ color: GREEN }}>THE NORTH STAR</div>
                <p className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.55)' }}>
                  A citizen opens the 156 app and the city already knows who they are, what they need, and how to help — seamlessly, across all 30+ agencies.
                </p>
              </div>
            </div>
          </motion.div>
        ...
        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(255,255,255,0.3)' }}>06 · FUTURE VISION</span>
        </div>
      </motion.div>
    </div>
  )
