      ...
      >
        ...
        <div className="relative z-10 w-[48%] flex flex-col justify-between px-14 lg:px-16 py-14 border-r"
          ...
          <div className="flex flex-col gap-6">
            ...
            <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.7 }}
              ...
              Turning São Paulo's distributed data into integrated city intelligence — one agency at a time.
            </motion.p>
          </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center px-12 py-14 gap-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.7 }}
            ...
            {[
              { value: '12M+', label: 'Residents', color: BLUE },
              { value: '30+', label: 'Agencies', color: CYAN },
              { value: '100+', label: 'Systems', color: BLUE },
            ].map((stat, i) => (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.85, duration: 0.7 }}
            ...
            <div className="flex items-center gap-3 pb-3" style={{ borderBottom: '1px solid rgba(47,128,255,0.12)' }}>
              <span className="text-xs font-mono font-semibold tracking-widest" style={{ color: CYAN }}>STRATEGIC PILLARS</span>
            </div>
            {[
              '🔗  Federated data lakes per agency',
              '🏛️  One government, one citizen view',
              '🔒  LGPD-compliant from day one',
              '☁️  Microsoft partnership for scale',
            ].map((item, i) => (
        ...
        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(255,255,255,0.3)' }}>01 · COVER</span>
        </div>
      </motion.div>
    </div>
  )
