    ...
    <div className="w-full h-full flex items-center justify-center p-6 lg:p-10">
      ...
      >
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center px-10 lg:px-12 py-14 gap-6">
          ...
          <div className="grid grid-cols-4 gap-3">
            {secretarias.map((s, i) => (
              <motion.div key={s.name}
                ...
                {s.active && (
                  <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ background: `${s.color}15`, color: s.color }}>运行中</span>
                )}
              </motion.div>
            ))}
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.3 }}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-xs font-mono font-bold tracking-wider mb-1" style={{ color: GREEN }}>北极星目标</div>
                <p className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.55)' }}>
                  市民打开156应用，城市已知道他们是谁、需要什么、如何提供帮助 — 跨越所有30+个机构，无缝衔接。
                </p>
              </div>
            </div>
          </motion.div>
        ...
        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(255,255,255,0.3)' }}>06 · 未来愿景</span>
        </div>
      </motion.div>
    </div>
  )
