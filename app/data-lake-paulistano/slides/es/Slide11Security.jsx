'use client'
import { motion } from 'framer-motion'

const BLUE = '#2F80FF'
const CYAN = '#00E5FF'
const PURPLE = '#A78BFA'

const principles = [
  {
    icon: '🔀',
    title: 'Réplica, No Migración',
    desc: 'Las bases de datos fuente nunca se modifican. Se crea una réplica de solo lectura y controlada en la capa del data lake — el sistema original sigue funcionando tal cual.',
    color: BLUE,
  },
  {
    icon: '🚧',
    title: 'Sin Movimiento Permanente de Datos',
    desc: 'Los datos replicados son transitorios y están regidos por calendarios de retención. Ningún registro ciudadano abandona permanentemente su sistema fuente.',
    color: CYAN,
  },
  {
    icon: '🔑',
    title: 'Control de Acceso Basado en Roles',
    desc: 'Cada consulta se autentica y autoriza. Los analistas solo ven datos agregados; los datos personales en bruto son accesibles únicamente mediante rutas explícitamente aprobadas.',
    color: PURPLE,
  },
  {
    icon: '🔍',
    title: 'Auditoría Completa de Consultas',
    desc: 'Cada evento de acceso a datos se registra de forma inmutable — quién accedió a qué, cuándo, desde dónde y con qué finalidad declarada.',
    color: BLUE,
  },
  {
    icon: '🌐',
    title: 'Residencia de Datos Garantizada',
    desc: 'Todos los datos permanecen en las regiones Azure de Brasil. Ningún dato cruza fronteras nacionales en ningún punto del flujo.',
    color: CYAN,
  },
  {
    icon: '🧱',
    title: 'Aislamiento por Diseño',
    desc: 'El data lake de cada secretaría está aislado de los demás por defecto. El acceso entre secretarías requiere aprobación de gobernanza explícita por caso de uso.',
    color: PURPLE,
  },
]

export default function Slide11Security() {
  return (
    <div className="w-full h-full flex items-center justify-center p-6 lg:p-10">
      <motion.div
        initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
        className="relative w-full h-full max-w-[1400px] max-h-[820px] rounded-3xl overflow-hidden flex flex-col"
        style={{ background: '#05070A', border: '1px solid rgba(47,128,255,0.1)' }}
      >
        <div className="absolute inset-0 pointer-events-none opacity-[0.025]"
          style={{ backgroundImage: `linear-gradient(${BLUE} 1px, transparent 1px), linear-gradient(90deg, ${BLUE} 1px, transparent 1px)`, backgroundSize: '60px 60px' }} />

        {/* Header */}
        <div className="relative z-10 flex items-center justify-between px-12 pt-8 pb-5 border-b"
          style={{ borderColor: 'rgba(47,128,255,0.1)' }}>
          <div>
            <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
              className="text-xs font-mono font-bold tracking-[0.25em] uppercase block mb-1" style={{ color: PURPLE }}>
              Seguridad de Datos
            </motion.span>
            <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
              className="font-heading font-bold text-white" style={{ fontSize: '1.9rem' }}>
              Los datos nunca abandonan la fuente — réplicas, no copias
            </motion.h2>
          </div>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            className="flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-mono tracking-wider"
            style={{ border: `1px solid ${PURPLE}30`, background: `${PURPLE}08`, color: PURPLE }}>
            <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: PURPLE }} />
            ARQUITECTURA ZERO TRUST
          </motion.div>
        </div>

        {/* Grid */}
        <div className="relative z-10 flex-1 grid grid-cols-3 grid-rows-2 gap-0">
          {principles.map((p, i) => (
            <motion.div key={p.title}
              initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
              transition={{ delay: 0.4 + i * 0.1, duration: 0.5 }}
              className="flex flex-col gap-3 p-7 relative overflow-hidden border-r border-b last:border-b-0"
              style={{
                borderColor: 'rgba(47,128,255,0.08)',
                borderRight: i % 3 === 2 ? 'none' : undefined,
                borderBottom: i >= 3 ? 'none' : undefined,
              }}>
              <div className="absolute bottom-0 left-0 right-0 h-20 pointer-events-none"
                style={{ background: `radial-gradient(ellipse at 50% 100%, ${p.color}05 0%, transparent 70%)` }} />
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl"
                style={{ background: `${p.color}12`, border: `1px solid ${p.color}25` }}>
                {p.icon}
              </div>
              <div>
                <div className="font-semibold text-sm mb-1.5" style={{ color: p.color }}>{p.title}</div>
                <p className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.45)' }}>{p.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>

        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(255,255,255,0.3)' }}>11 · SEGURIDAD</span>
        </div>
      </motion.div>
    </div>
  )
}
