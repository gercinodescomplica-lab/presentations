...
const principles = [
  { icon: '🏗️', title: 'Arquitectura Federada', desc: 'Cada secretaría tiene un Data Lake dedicado conectado a una red de alcance municipal.', color: BLUE },
  { icon: '🔄', title: 'Integración No Invasiva', desc: 'Los datos se replican — no se mueven. Los sistemas fuente siguen funcionando sin cambios.', color: CYAN },
  { icon: '🔍', title: 'Analítica Unificada', desc: 'Consultas e insights entre secretarías sin centralizar los datos en bruto.', color: BLUE },
  { icon: '🛡️', title: 'Privacidad por Diseño', desc: 'Cumplimiento LGPD incorporado en cada capa, desde la ingesta hasta el control de acceso.', color: CYAN },
]
      ...
      >
        ...
        <div className="relative z-10 w-[42%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-6 border-r"
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.7 }}
            ...
            Una ciudad.<br />Una arquitectura de datos.
          </motion.h2>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.65 }}
            ...
            El Data Lake Paulistano es una red federada de data lakes a nivel de secretaría, cada uno administrado y gobernado por su respectiva secretaría — pero conectado a una capa de inteligencia compartida que abarca toda la ciudad.
          </motion.p>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}
            className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.45)' }}>
            Piénsalo como una constelación: cada estrella brilla de forma independiente, pero juntas iluminan un mapa completo de São Paulo.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }}
            ...
            <span className="text-xs font-mono tracking-widest" style={{ color: CYAN }}>PILOTO ACTIVO · SMIT</span>
          </motion.div>
        </div>
        ...
        <div className="absolute top-6 right-8 z-20">
          <span className="text-xs font-mono tracking-widest" style={{ color: 'rgba(255,255,255,0.3)' }}>03 · CONCEPTO</span>
        </div>
      </motion.div>
    </div>
  )
