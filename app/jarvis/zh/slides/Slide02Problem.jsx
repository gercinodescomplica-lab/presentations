...
const pains = [
  { icon: LayoutGrid, title: '4个不同的系统', desc: '只为回答一个简单问题。频繁切换系统严重影响工作效率。' },
  { icon: FileSearch, title: '手动查找合同', desc: '逐张表格查找本月到期的合同，耗时费力。' },
  { icon: BellOff, title: '遗忘的跟进事项', desc: '没有自动提醒。会议结束，承诺就此消失。' },
  { icon: FolderSearch, title: '找不到的PDF', desc: '在邮件中翻找数周前发送的文件。' },
  { icon: Clock, title: '碎片化的注意力', desc: '日历、项目、邮件各在一个应用 — 三个应用，浪费一小时。' },
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[38%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-5 border-r border-slate-100">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
            ...
            <span className="text-slate-400 text-sm tracking-wide">碎片化的代价</span>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
            className="text-slate-500 text-sm leading-relaxed">
            管理者和高管每天在本应几秒钟完成的任务上浪费数小时。每个答案都需要手动打开多个系统查找。
          </motion.p>
