...
const pillars = [
  { icon: Smartphone, label: '无需新应用', desc: '直接使用您的团队全天都开着的WhatsApp即可。' },
  { icon: MessageCircle, label: '文字或语音', desc: '输入问题或发送语音消息 — Jarvis两种都能理解。' },
  { icon: Mic, label: '零培训成本', desc: '只要会发WhatsApp消息，就能使用Jarvis。' },
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[45%] flex flex-col justify-center px-12 lg:px-16 py-14 gap-6 border-r border-slate-100 relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
            ...
            <span className="text-slate-400 text-sm tracking-wide">Jarvis就在您的WhatsApp里</span>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }}
            className="text-slate-500 text-base leading-relaxed">
            <strong className="text-slate-700">Jarvis</strong> 是直接在WhatsApp中提供的AI行政助理 — 您的管理者整天都在使用的应用。连接到企业真实数据：合同、项目、日历、邮件和文件。
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
            ...
            <span className="text-xs font-mono font-semibold tracking-widest" style={{ color: '#16A34A' }}>实时运行中</span>
          </motion.div>
        </div>
