...
const pillars = [
  { icon: Smartphone, label: 'Без нового приложения', desc: 'Работает в WhatsApp, который ваша команда уже держит открытым весь день.' },
  { icon: MessageCircle, label: 'Текст или голос', desc: 'Напишите вопрос или отправьте аудио — Jarvis понимает оба формата.' },
  { icon: Mic, label: 'Ноль обучения', desc: 'Если вы умеете отправлять сообщения в WhatsApp, вы умеете пользоваться Jarvis.' },
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[45%] flex flex-col justify-center px-12 lg:px-16 py-14 gap-6 border-r border-slate-100 relative z-10">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            ...
            Одно сообщение.<br />Мгновенный ответ.
          </motion.h2>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
            className="flex items-center gap-3">
            <div className="h-[3px] w-8 rounded-full" style={{ background: `linear-gradient(90deg, ${V}, ${G})` }} />
            <span className="text-slate-400 text-sm tracking-wide">Jarvis живёт в вашем WhatsApp</span>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }}
            className="text-slate-500 text-base leading-relaxed">
            <strong className="text-slate-700">Jarvis</strong> — AI-ассистент для руководителей, доступный прямо в WhatsApp — приложении, которым ваши менеджеры пользуются весь день. Подключён к реальным данным компании: контракты, проекты, календарь, почта и документы.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
            ...
            <span className="text-xs font-mono font-semibold tracking-widest" style={{ color: '#16A34A' }}>В ЭФИРЕ · РАБОТАЕТ</span>
          </motion.div>
        </div>
