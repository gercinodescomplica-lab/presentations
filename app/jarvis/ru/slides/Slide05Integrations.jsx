...
const integrations = [
  { icon: MessageCircle, name: 'WhatsApp', sub: 'Evolution API', desc: 'Текст, аудио, PDF — основной канал', color: '#25D366' },
  { icon: BookOpen, name: 'Notion', sub: 'Projects API', desc: 'Проекты, задачи, статусы и дедлайны', color: '#000000' },
  { icon: Mail, name: 'Microsoft 365', sub: 'MS Graph', desc: 'Календарь, встречи и почта', color: '#0078D4' },
  { icon: FileText, name: 'Контракты', sub: 'Внутренняя система', desc: 'Суммы, сроки, менеджеры и объекты', color: '#7C3AED' },
  { icon: BarChart3, name: 'Дэшборд DRM', sub: 'Коммерческий', desc: 'Цели, прогноз, воронка и CX', color: '#F59E0B' },
  { icon: File, name: 'Документы', sub: 'Семантический поиск PDF', desc: 'Индексированный и доступный для поиска контент', color: '#EF4444' },
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[35%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-5 border-r border-slate-100 relative z-10">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            ...
            Подключён к тому, что важно
          </motion.h2>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }}
            className="flex items-center gap-3">
            <div className="h-[3px] w-8 rounded-full" style={{ background: `linear-gradient(90deg, ${V}, #25D366)` }} />
            <span className="text-slate-400 text-sm tracking-wide">6 систем, один разговор</span>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }}
            className="text-slate-500 text-sm leading-relaxed">
            Jarvis подключается напрямую к вашим ключевым системам — без копирования, без ручных запросов. Каждый ответ приходит из живого авторитетного источника.
          </motion.p>
        </div>
