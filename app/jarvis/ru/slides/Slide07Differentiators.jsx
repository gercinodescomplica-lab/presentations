...
const diffs = [
  { icon: History, color: '#7C3AED', title: 'Постоянный контекст', desc: 'Jarvis помнит, что говорилось в предыдущих разговорах. Вам никогда не придётся повторять контекст — он продолжит с того места, где остановился.' },
  { icon: Mic2, color: '#25D366', title: 'Голосовой ввод', desc: 'Говорите вместо того, чтобы печатать. Jarvis расшифровывает, понимает и отвечает — без лишних шагов.' },
  { icon: ShieldCheck, color: '#0EA5E9', title: 'Всегда точен', desc: 'Каждый ответ приходит из реального источника — API, базы данных или индексированного документа. При неуверенности Jarvis так и скажет.' },
  { icon: Users, color: '#F59E0B', title: 'Поддержка групп', desc: 'Добавьте Jarvis в группы WhatsApp. Он отвечает при упоминании @Jarvis — идеально для координации команды.' },
  { icon: Bell, color: '#EF4444', title: 'Умные напоминания', desc: '"Напомни подписать дополнение SMADS во вторник в 14:00" — Jarvis отправит уведомление в нужное время, даже в оффлайне.' },
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[35%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-5 border-r border-slate-100 relative z-10">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            ...
            Почему Jarvis другой
          </motion.h2>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }}
            className="flex items-center gap-3">
            <div className="h-[3px] w-8 rounded-full" style={{ background: `linear-gradient(90deg, ${V}, #25D366)` }} />
            <span className="text-slate-400 text-sm tracking-wide">Не чатбот. Руководитель.</span>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }}
            className="text-slate-500 text-sm leading-relaxed">
            Большинство AI-инструментов отвечают на общие вопросы. Jarvis знает данные вашей компании, вашу историю и расписание — и даёт ответы, которые действительно важны.
          </motion.p>
        </div>
