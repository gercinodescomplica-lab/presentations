...
const done = [
  'WhatsApp — текст, аудио и PDF',
  'Контракты с предвычисленной аналитикой',
  'Интеграция с проектами Notion',
  'Microsoft Graph — календарь',
  'Microsoft 365 — электронная почта',
  'Постоянная семантическая память',
  'Напоминания в реальном времени через Trigger.dev',
  'Индексация PDF и семантический поиск',
  'Поддержка групп WhatsApp (@Jarvis)',
]

const next = [
  'Проактивные уведомления об истечении контрактов',
  'Веб-дэшборд аналитики',
  'Интеграция с цифровой подписью',
  'Нативное мобильное приложение (альтернативный канал)',
  'Мультитенантность (другие дирекции)',
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[32%] flex flex-col justify-center px-10 lg:px-12 py-14 gap-5 border-r border-slate-100 relative z-10">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            ...
            <span className="text-xs font-mono font-bold tracking-widest" style={{ color: '#16A34A' }}>РАБОТАЕТ</span>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
            className="text-slate-500 text-sm leading-relaxed">
            Все основные функции активны и используются. 9 возможностей развёрнуты и проверены в production.
          </motion.p>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.75 }}
            className="flex gap-6">
            <div>
              <div className="text-3xl font-black text-slate-900">9</div>
              <div className="text-xs text-slate-400 font-mono uppercase tracking-wider">Развёрнуто</div>
            </div>
            <div>
              <div className="text-3xl font-black" style={{ color: V }}>5</div>
              <div className="text-xs text-slate-400 font-mono uppercase tracking-wider">Роадмап</div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="flex-1 flex gap-0 relative z-10">
          <div className="flex-1 flex flex-col justify-center px-8 py-12 gap-2.5 border-r border-slate-100">
            <div className="text-xs font-mono font-bold tracking-widest uppercase text-slate-400 mb-2">✅ Развёрнуто</div>
            {done.map((item, i) => (
          ...
          <div className="flex-1 flex flex-col justify-center px-8 py-12 gap-2.5">
            <div className="text-xs font-mono font-bold tracking-widest uppercase text-slate-400 mb-2">⬜ Следующие шаги</div>
            {next.map((item, i) => (
              <motion.div key={item} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}
