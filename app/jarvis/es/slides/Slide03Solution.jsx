...
const pillars = [
  { icon: Smartphone, label: 'Sin nueva app', desc: 'Funciona en el WhatsApp que tu equipo ya tiene abierto todo el día.' },
  { icon: MessageCircle, label: 'Texto o voz', desc: 'Escribe una pregunta o envía un audio — Jarvis entiende ambos.' },
  { icon: Mic, label: 'Cero formación', desc: 'Si puedes enviar un mensaje de WhatsApp, puedes usar Jarvis.' },
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[45%] flex flex-col justify-center px-12 lg:px-16 py-14 gap-6 border-r border-slate-100 relative z-10">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            ...
            Un mensaje.<br />Respuesta inmediata.
          </motion.h2>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
            className="flex items-center gap-3">
            <div className="h-[3px] w-8 rounded-full" style={{ background: `linear-gradient(90deg, ${V}, ${G})` }} />
            <span className="text-slate-400 text-sm tracking-wide">Jarvis vive en WhatsApp</span>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }}
            className="text-slate-500 text-base leading-relaxed">
            <strong className="text-slate-700">Jarvis</strong> es un asistente ejecutivo con IA disponible directamente en WhatsApp — la app que tus gestores usan todo el día. Conectado a los datos reales de tu empresa: contratos, proyectos, agenda, correo y documentos.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
            ...
            <span className="text-xs font-mono font-semibold tracking-widest" style={{ color: '#16A34A' }}>EN VIVO · OPERACIONAL</span>
          </motion.div>
        </div>
