...
const integrations = [
  { icon: MessageCircle, name: 'WhatsApp', sub: 'Evolution API', desc: 'Texto, audio, PDFs — el canal principal', color: '#25D366' },
  { icon: BookOpen, name: 'Notion', sub: 'Projects API', desc: 'Proyectos, tareas, estado y plazos', color: '#000000' },
  { icon: Mail, name: 'Microsoft 365', sub: 'MS Graph', desc: 'Agenda, reuniones y correos', color: '#0078D4' },
  { icon: FileText, name: 'Contratos', sub: 'Sistema Interno', desc: 'Valores, vencimientos, gestores y objetos', color: '#7C3AED' },
  { icon: BarChart3, name: 'Dashboard DRM', sub: 'Comercial', desc: 'Metas, forecast, pipeline y CX', color: '#F59E0B' },
  { icon: File, name: 'Documentos', sub: 'Búsqueda Semántica PDF', desc: 'Contenido indexado y buscable', color: '#EF4444' },
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[35%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-5 border-r border-slate-100 relative z-10">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            ...
            Conectado a lo que importa
          </motion.h2>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }}
            className="flex items-center gap-3">
            <div className="h-[3px] w-8 rounded-full" style={{ background: `linear-gradient(90deg, ${V}, #25D366)` }} />
            <span className="text-slate-400 text-sm tracking-wide">6 sistemas, una conversación</span>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }}
            className="text-slate-500 text-sm leading-relaxed">
            Jarvis se conecta directamente a tus sistemas críticos — sin copiar y pegar, sin consultas manuales. Cada respuesta proviene de una fuente viva y oficial.
          </motion.p>
        </div>
