...
const done = [
  'WhatsApp — texto, audio y PDF',
  'Contratos con analíticas pre-calculadas',
  'Integración con proyectos de Notion',
  'Microsoft Graph — agenda',
  'Microsoft 365 — correos',
  'Memoria semántica persistente',
  'Recordatorios en tiempo real vía Trigger.dev',
  'Indexación de PDFs y búsqueda semántica',
  'Soporte a grupos de WhatsApp (@Jarvis)',
]

const next = [
  'Alertas proactivas de vencimiento de contratos',
  'Dashboard web de analíticas',
  'Integración con firma digital',
  'App móvil nativa (canal alternativo)',
  'Multi-tenant (otras direcciones)',
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[32%] flex flex-col justify-center px-10 lg:px-12 py-14 gap-5 border-r border-slate-100 relative z-10">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
            ...
            Todas las funciones centrales están activas y en uso. 9 capacidades desplegadas y validadas en producción.
          </motion.p>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.75 }}
            className="flex gap-6">
            <div>
              <div className="text-3xl font-black text-slate-900">9</div>
              <div className="text-xs text-slate-400 font-mono uppercase tracking-wider">Desplegados</div>
            </div>
            <div>
        ...
        <div className="flex-1 flex gap-0 relative z-10">
          <div className="flex-1 flex flex-col justify-center px-8 py-12 gap-2.5 border-r border-slate-100">
            <div className="text-xs font-mono font-bold tracking-widest uppercase text-slate-400 mb-2">✅ Desplegado</div>
            {done.map((item, i) => (
          ...
          <div className="flex-1 flex flex-col justify-center px-8 py-12 gap-2.5">
            <div className="text-xs font-mono font-bold tracking-widest uppercase text-slate-400 mb-2">⬜ Próximos Pasos</div>
            {next.map((item, i) => (
              <motion.div key={item} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}
