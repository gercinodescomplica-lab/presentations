...
const reasons = [
  { icon: Cpu, color: '#7C3AED', title: 'La IA alcanzó el punto de inflexión', desc: 'Los LLMs ahora entienden contexto, llaman APIs reales y razonan sobre datos en vivo — sin alucinaciones cuando están bien configurados.' },
  { icon: Smartphone, color: '#25D366', title: '98% de penetración de WhatsApp en Brasil', desc: 'Es la única app que todos tienen abierta todo el día. El canal ya está instalado — barrera de adopción cero.' },
  { icon: Zap, color: '#F59E0B', title: 'IA + canal + datos reales = trabajo real sustituido', desc: 'No es un chatbot de FAQ. Es un asistente que genuinamente reemplaza trabajo manual, repetitivo y que consume tiempo.' },
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="text-center relative z-10">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            ...
            El momento exacto ha llegado.
          </motion.h2>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }}
            className="text-slate-400 text-lg max-w-xl mx-auto">
            Tres fuerzas se alinean — y la ventana está abierta.
          </motion.p>
        </div>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
          ...
          Jarvis — construido para quienes no tienen tiempo que perder.
        </motion.p>
      </motion.div>
    </div>
  )
