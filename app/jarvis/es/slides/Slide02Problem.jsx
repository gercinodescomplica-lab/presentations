...
const pains = [
  { icon: LayoutGrid, title: '4 sistemas diferentes', desc: 'Para responder una pregunta sencilla. El cambio de contexto destruye la productividad.' },
  { icon: FileSearch, title: 'Búsqueda manual de contratos', desc: 'Hoja de cálculo por hoja de cálculo para saber qué contratos vencen este mes.' },
  { icon: BellOff, title: 'Seguimientos olvidados', desc: 'Sin recordatorio automático. La reunión termina y los compromisos desaparecen.' },
  { icon: FolderSearch, title: 'PDFs perdidos', desc: 'Buscando en correos para encontrar un documento enviado hace semanas.' },
  { icon: Clock, title: 'Atención fragmentada', desc: 'Agenda, proyectos y correo — tres apps separadas, una hora desperdiciada.' },
]
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-[38%] flex flex-col justify-center px-12 lg:px-14 py-14 gap-5 border-r border-slate-100">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            ...
            Horas perdidas en sistemas — cada día
          </motion.h2>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
            className="flex items-center gap-3">
            <div className="h-[3px] w-8 rounded-full" style={{ background: '#EF4444' }} />
            <span className="text-slate-400 text-sm tracking-wide">El costo de la fragmentación</span>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
            className="text-slate-500 text-sm leading-relaxed">
            Gestores y ejecutivos pierden horas al día en tareas que deberían tomar segundos. Cada respuesta requiere abrir múltiples sistemas — manualmente.
          </motion.p>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
            ...
            <p className="text-xs text-slate-500 italic leading-relaxed">
              "El tiempo de un gestor es demasiado valioso para gastarlo navegando sistemas."
            </p>
          </motion.div>
        </div>
