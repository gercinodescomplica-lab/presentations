...
const chat = [
  { from: 'user', text: 'Qual contrato vence esse mês?' },
  { from: 'jarvis', text: '2 contratos vencem em abril:\n• SEGES — R$ 45,1mi — vence 30/04\n• SGM — R$ 25,7mi — vence 30/04' },
  { from: 'user', text: 'Me lembra de ligar pro Magrão na sexta às 9h' },
  { from: 'jarvis', text: '⏰ Lembrete criado! Te aviso sexta às 09:00.' },
]
      ...
      >
        ...
        <div className="w-1/2 flex flex-col justify-center px-14 lg:px-16 py-14 gap-6 relative z-10">
          ...
          <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.6 }}
            ...
            O assistente executivo que vive no seu <strong className="text-slate-700 font-semibold">WhatsApp</strong>.
          </motion.p>
          ...
          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }}
            ...
            Sem novo app. Sem nova senha. Sem treinamento.<br />
            Basta mandar uma mensagem — ou um áudio.
          </motion.p>

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}
            className="flex items-center gap-3 flex-wrap">
            {[{ icon: Bot, label: 'Gemini' }, { icon: MessageCircle, label: 'WhatsApp' }, { icon: Zap, label: 'Tempo Real' }].map(({ icon: Icon, label }) => (
