  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-6 lg:p-10">
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="relative w-full h-full max-w-[1400px] max-h-[820px] rounded-3xl overflow-hidden flex flex-col items-center justify-center gap-10 px-16 bg-white" style={{ border: '1px solid rgba(26,86,219,0.12)', boxShadow: '0 20px 80px -20px rgba(26,86,219,0.14)' }}>
        ...
        <div className="absolute top-6 left-8 flex items-center gap-2 z-20"><span className="text-slate-300 text-xs font-mono tracking-widest">07</span><span className="text-slate-200 text-xs font-mono">·</span><span className="text-slate-300 text-xs font-mono tracking-widest">СЛЕДУЮЩИЕ ШАГИ</span></div>
        <div className="relative z-10 flex flex-col items-center gap-4 text-center">
          ...
          <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4, ease: [0.22, 1, 0.36, 1] }} className="font-heading font-bold leading-tight text-center" style={{ fontSize: 'clamp(1.8rem, 3.2vw, 2.8rem)', background: `linear-gradient(135deg, #0f172a 0%, ${BLUE} 60%, ${GREEN} 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.95 }} className="relative z-10 flex flex-col items-center gap-4 max-w-2xl text-center">
          ...
          <p className="font-heading font-light text-lg italic leading-relaxed text-slate-500">«Когда видимость высока и ошибка недопустима, PRODAM — правильный выбор.»</p>
