...
const stats = [
  { value: '+20', label: 'Выборов проведено', color: BLUE },
  { value: '690K', label: 'Избирателей обслужено', color: GREEN },
...
export default function Slide05Rezultaty() {
  return (
    <SlideCardLight slideNum="05" label="РЕЗУЛЬТАТЫ">
      <div className="flex h-full">
        <LeftPanelLight>
          <TagLabelLight color={GOLD}>Результаты</TagLabelLight>
          <GradientTitleLight>Результаты, которые<br />подтверждают<br />решение</GradientTitleLight>
          <SubtitleBarLight>Доказанный опыт в Сан-Паулу</SubtitleBarLight>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="rounded-xl p-4 flex flex-col gap-2" style={{ background: `${BLUE}06`, border: `1px solid ${BLUE}18` }}>
            <span className="text-[10px] font-mono font-semibold tracking-wider" style={{ color: BLUE }}>ДОСТИЖЕНИЯ 2025</span>
            <ul className="flex flex-col gap-1.5">
              {['6 выборов высокой сложности', 'Новый административный модуль сдан', 'Новая онлайн-урна внедрена'].map((item, i) => (
        ...
        <RightPanelLight>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }} className="flex flex-col gap-3 w-full">
            <span className="text-[10px] font-mono font-semibold tracking-widest text-slate-400">КЛИЕНТЫ-РЕФЕРЕНСЫ</span>
