  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-6 lg:p-10">
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="relative w-full h-full max-w-[1400px] max-h-[820px] rounded-3xl overflow-hidden flex bg-white" style={{ border: '1px solid rgba(26,86,219,0.12)', boxShadow: '0 20px 80px -20px rgba(26,86,219,0.14)' }}>
        ...
        <div className="relative z-10 w-1/2 flex flex-col justify-between px-14 lg:px-16 py-14 bg-slate-50/60 border-r border-slate-100">
          <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.3 }}>
            <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full w-fit text-xs font-mono tracking-wider" style={{ border: `1px solid ${BLUE}30`, background: `${BLUE}08`, color: BLUE }}>
              <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: BLUE }} />PITCH DE VENTAS · DRM
            </div>
          </motion.div>
          <div className="flex flex-col gap-6">
            ...
            <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.5, ease: [0.22, 1, 0.36, 1] }} className="font-heading font-bold leading-none tracking-tight" style={{ fontSize: 'clamp(2.4rem, 4.5vw, 3.8rem)', background: `linear-gradient(135deg, #0f172a 0%, ${BLUE} 65%, ${GREEN} 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              PRODAM Elecciones:<br />Tecnología que<br />garantiza cada voto
            </motion.h1>
            ...
            <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.85 }} className="text-slate-500 text-lg leading-relaxed font-light">
              Solución completa de gestión electoral para organismos públicos municipales
            </motion.p>
          </div>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.2 }} className="flex flex-col gap-2">
            <div className="flex items-center gap-3"><div className="w-1.5 h-1.5 rounded-full" style={{ background: GOLD }} /><span className="text-slate-400 text-xs font-mono tracking-widest">DRM · POSTVENTA Y PRODUCTOS SaaS</span></div>
        ...
        <div className="relative z-10 flex-1 flex flex-col py-14 px-10 gap-6 bg-white">
          <motion.div initial={{ opacity: 0, scale: 0.94, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.55, ease: [0.22, 1, 0.36, 1] }} className="rounded-2xl p-8 flex flex-col gap-5 flex-1" style={{ background: '#F8FAFF', border: `1px solid ${BLUE}18`, boxShadow: '0 8px 32px -8px rgba(26,86,219,0.10)' }}>
            <div className="flex items-center gap-3 pb-4" style={{ borderBottom: `1px solid ${BLUE}15` }}>
              <span className="text-2xl">🗳️</span><span className="text-slate-400 text-sm font-mono tracking-wider font-semibold">SISTEMA ELECTORAL</span>
            </div>
            {['Urna en Línea', 'Urna Presencial', 'Escrutinio TSE', 'Backoffice'].map((item, i) => (
          ...
          <div className="flex gap-4">
            {[{ value: '+20', label: 'elecciones' }, { value: '690K', label: 'electores' }, { value: '1M+', label: 'votos' }].map((stat, i) => (
        ...
        <div className="absolute top-6 right-8 flex items-center gap-2 z-20"><span className="text-slate-300 text-xs font-mono tracking-widest">01</span><span className="text-slate-200 text-xs font-mono">·</span><span className="text-slate-300 text-xs font-mono tracking-widest">PORTADA</span></div>
      </motion.div>
    </div>
  )
}
