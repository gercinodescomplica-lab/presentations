...
const steps = [
  { number: '01', icon: '🔍', title: 'Diagnóstico', desc: 'Comprender el modelo electoral del organismo y mapear necesidades específicas.', color: BLUE },
  { number: '02', icon: '🖥️', title: 'Demostración', desc: 'Presentar la plataforma en funcionamiento real con escenarios de su proceso.', color: GREEN },
  { number: '03', icon: '📋', title: 'Propuesta', desc: 'Estructurar la operación completa con SLA garantizado y cronograma definido.', color: GOLD },
]

export default function Slide07ProximosPasos() {
  return (
    <div className="w-full h-full flex items-center justify-center p-6 lg:p-10">
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="relative w-full h-full max-w-[1400px] max-h-[820px] rounded-3xl overflow-hidden flex flex-col items-center justify-center gap-10 px-16 bg-white" style={{ border: '1px solid rgba(26,86,219,0.12)', boxShadow: '0 20px 80px -20px rgba(26,86,219,0.14)' }}>
        ...
        <div className="absolute top-6 left-8 flex items-center gap-2 z-20"><span className="text-slate-300 text-xs font-mono tracking-widest">07</span><span className="text-slate-200 text-xs font-mono">·</span><span className="text-slate-300 text-xs font-mono tracking-widest">PRÓXIMOS PASOS</span></div>
        <div className="relative z-10 flex flex-col items-center gap-4 text-center">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-mono font-semibold tracking-wider" style={{ border: `1px solid ${GREEN}30`, background: `${GREEN}08`, color: GREEN }}>
            <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: GREEN }} />PRÓXIMOS PASOS
          </motion.div>
          <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4, ease: [0.22, 1, 0.36, 1] }} className="font-heading font-bold leading-tight text-center" style={{ fontSize: 'clamp(2rem, 3.8vw, 3rem)', background: `linear-gradient(135deg, #0f172a 0%, ${BLUE} 60%, ${GREEN} 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
            ¿Garantizamos el éxito<br />de su próxima elección?
          </motion.h2>
        </div>
        <div className="relative z-10 flex gap-6 w-full max-w-3xl">
          {steps.map(({ number, icon, title, desc, color }, i) => (
            <motion.div key={title} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.55 + i * 0.12, ease: [0.22, 1, 0.36, 1] }} className="flex-1 flex flex-col gap-4 p-6 rounded-2xl bg-white relative overflow-hidden" style={{ border: `1px solid ${color}18`, boxShadow: '0 8px 32px -8px rgba(0,0,0,0.08)' }}>
              ...
              <div className="flex flex-col gap-1.5"><span className="font-semibold text-lg" style={{ color }}>{title}</span><p className="text-slate-500 text-base leading-relaxed">{desc}</p></div>
            </motion.div>
          ))}
        </div>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.95 }} className="relative z-10 flex flex-col items-center gap-4 max-w-2xl text-center">
          ...
          <p className="font-heading font-light text-lg italic leading-relaxed text-slate-500">"Cuando la visibilidad es alta y el error no es opción,<br />PRODAM es la elección correcta."</p>
