...
const stats = [
  { value: '+20', label: '次成功选举', color: BLUE },
  { value: '690K', label: '名选民服务', color: GREEN },
  { value: '+1M', label: '张选票处理', color: GOLD },
]

const clients = ['CMDCA', 'IPREM', 'COMAS', 'CMH', 'CPM', 'CMPU', 'SMADS', 'SEHAB', 'PRODAM']

export default function Slide05Xinren() {
  return (
    <SlideCardLight slideNum="05" label="信任与成果">
      <div className="flex h-full">
        <LeftPanelLight>
          <TagLabelLight color={GOLD}>成果证明</TagLabelLight>
          <GradientTitleLight>用成果证明<br />解决方案</GradientTitleLight>
          <SubtitleBarLight>圣保罗市政府的信赖选择</SubtitleBarLight>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="rounded-xl p-4 flex flex-col gap-2" style={{ background: `${BLUE}06`, border: `1px solid ${BLUE}18` }}>
            <span className="text-[10px] font-mono font-semibold tracking-wider" style={{ color: BLUE }}>2025年度亮点</span>
            <ul className="flex flex-col gap-1.5">
              {['6个高复杂度选举流程', '新管理模块成功交付', '全新在线投票系统上线'].map((item, i) => (
        ...
        <RightPanelLight>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }} className="flex flex-col gap-3 w-full">
            <span className="text-[10px] font-mono font-semibold tracking-widest text-slate-400">参考客户</span>
