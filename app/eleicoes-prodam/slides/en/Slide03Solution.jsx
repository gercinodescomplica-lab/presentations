...
const modules = [
  { icon: '🖥️', name: 'Online Ballot', desc: 'Remote voting integrated with Microsoft 365', color: BLUE, tag: 'Remote' },
  { icon: '🏛️', name: 'In-Person Ballot', desc: 'Poll worker replica system for large councils', color: GREEN, tag: 'Presential' },
  { icon: '📊', name: 'TSE Tabulation', desc: 'Reading electronic ballot reports from TSE', color: CYAN, tag: 'Integration' },
  { icon: '⚙️', name: 'Backoffice', desc: 'Autonomous management by the client', color: GOLD, tag: 'Management' },
]
            ...
            {modules.map(({ icon, name, desc, color, tag }, i) => (
              <motion.div key={name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 + i * 0.12, ease: [0.22, 1, 0.36, 1] }} className="flex flex-col gap-3 p-5 rounded-2xl relative overflow-hidden bg-white" style={{ border: `1px solid ${color}20`, boxShadow: '0 4px 20px -5px rgba(0,0,0,0.05)' }}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-mono font-semibold tracking-widest px-2 py-0.5 rounded-full" style={{ background: `${color}12`, color }}>{tag}</span>
                </div>
                <div className="flex flex-col gap-1">
