...
const points = [
  { icon: Bot, label: 'Atendimento automatizado', desc: 'Chatbots e assistentes virtuais que resolvem demandas sem intervenção humana.', color: '#09DFAB' },
  { icon: LineChart, label: 'Análise preditiva', desc: 'Modelos que antecipam demandas, crises e necessidades da população.', color: '#04A8B0' },
  { icon: Settings2, label: 'Eficiência operacional', desc: 'Automação de processos repetitivos, liberando servidores para tarefas de maior valor.', color: '#04767F' },
  { icon: HelpCircle, label: 'Apoio à decisão', desc: 'IA como suporte estratégico para gestores e formuladores de políticas públicas.', color: '#09DFAB' },
  { icon: Sparkles, label: 'Personalização do serviço', desc: 'Experiências adaptadas ao perfil e histórico de cada cidadão.', color: '#04A8B0' },
]

export default function Slide12IA() {
  return (
      ...
      <div className="w-[30%] h-full flex flex-col justify-between p-12 relative border-r border-[#1E3A52]" style={{ backgroundColor: '#0A1120' }}>
        ...
        <div className="relative z-10 flex flex-col gap-6">
          ...
          <div>
            ...
            >
              Inteligência<br />
              <span className="text-[#09DFAB]">Artificial</span><br />
              na gestão
            </motion.h2>
            <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="text-[#8BA0B4] text-sm leading-relaxed">
              Como a IA está transformando a capacidade de entrega do setor público.
            </motion.p>
          </div>
        </div>
        ...
        >
          ...
          <p className="text-white text-sm leading-snug font-medium italic">
            "A IA não substitui a gestão pública. Ela potencializa sua capacidade de entrega."
          </p>
        </motion.div>
      </div>
