...
const takeaways = [
  { num: '01', text: 'Transformação digital é inevitável', sub: 'Governos que não se digitalizam perdem eficiência e relevância.' },
  { num: '02', text: 'Integração gera eficiência', sub: 'Sistemas conectados eliminam redundâncias e aceleram o atendimento.' },
  { num: '03', text: 'Interoperabilidade gera inteligência', sub: 'Dados cruzados entre órgãos produzem insights impossíveis em silos.' },
  { num: '04', text: 'Ecossistemas aceleram inovação', sub: 'Nenhuma entidade pública inova sozinha — parcerias são a chave.' },
  { num: '05', text: 'O cidadão deve estar no centro', sub: 'Toda decisão tecnológica deve partir da experiência do cidadão.' },
]

export default function Slide14Conclusao() {
  return (
    <div className="w-full h-full flex bg-[#0F172A] text-white overflow-hidden">
      ...
      <div className="w-[30%] h-full flex flex-col justify-between p-12 relative border-r border-[#1E3A52]" style={{ backgroundColor: '#0A1120' }}>
        ...
        <div className="relative z-10 flex flex-col gap-6">
          ...
          <div>
            ...
            <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="text-[#8BA0B4] text-sm leading-relaxed">
              O que ficamos levando desta jornada de transformação digital pública.
            </motion.p>
          </div>
        </div>
        ...
        >
          <CheckCircle className="w-5 h-5 text-[#09DFAB] flex-shrink-0" />
          <span className="text-[#8BA0B4] text-sm">5 princípios fundamentais</span>
        </motion.div>
      </div>
      ...
      <div className="flex-1 h-full relative flex flex-col justify-center px-12 py-10 gap-3">
        ...
        <div className="relative z-10 flex flex-col gap-3">
          {takeaways.map((item, i) => (
            ...
            >
              <span
                className="font-mono font-bold text-xl flex-shrink-0 w-10"
                style={{ color: '#09DFAB', textShadow: '0 0 12px rgba(9,223,171,0.5)' }}
              >
                {item.num}
              </span>
              <div>
                <p className="text-white font-bold text-base mb-0.5">{item.text}</p>
                <p className="text-[#8BA0B4] text-sm leading-snug">{item.sub}</p>
              </div>
            </motion.div>
          ))}
        </div>
