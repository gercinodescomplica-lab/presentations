      ...
      <div className="w-[30%] h-full flex flex-col justify-between p-12 relative border-r border-[#1E3A52]" style={{ backgroundColor: '#0A1120' }}>
        ...
        <div className="relative z-10 flex flex-col gap-6">
          ...
          <div>
            ...
            >
              O que é<br />
              <span className="text-[#09DFAB]">interoperabi-<br />lidade?</span>
            </motion.h2>
          </div>
        </div>
        ...
        >
          <defs>
            <radialGradient id="hub-glow" cx="50%" cy="50%" r="50%">
              <stop offset="0%" stopColor="#09DFAB" stopOpacity="0.15" />
          ...
          {sectors.map((s) => {
            ...
            return (
              <line key={s.label} x1={cx} y1={cy} x2={pos.x} y2={pos.y}
                stroke="rgba(9,223,171,0.25)" strokeWidth="1.5" strokeDasharray="4 3" />
            )
          })}
          ...
          {sectors.map((s, i) => {
            const pos = polarToCartesian(cx, cy, r, s.angle)
            const labelPos = polarToCartesian(cx, cy, r + 38, s.angle)
            return (
              <g key={s.label}>
                <circle cx={pos.x} cy={pos.y} r={26} fill={`${s.color}18`} stroke={`${s.color}50`} strokeWidth="1.5" />
                <circle cx={pos.x} cy={pos.y} r={7} fill={s.color} filter="url(#glow-f)" />
                <text x={labelPos.x} y={labelPos.y} textAnchor="middle" dominantBaseline="middle"
                  fill="#8BA0B4" fontSize="13" fontFamily="system-ui,sans-serif" fontWeight="500">
                  {s.label}
                </text>
              </g>
            )
          ...
          {/* Hub central glow */}
          <circle cx={cx} cy={cy} r={65} fill="url(#hub-glow)" />
          <circle cx={cx} cy={cy} r={48} fill="rgba(9,223,171,0.06)" stroke="rgba(9,223,171,0.35)" strokeWidth="1.5" />
          <text x={cx} y={cy - 10} textAnchor="middle" fill="#09DFAB" fontSize="13" fontFamily="system-ui,sans-serif" fontWeight="800" letterSpacing="1">PLATA-</text>
          <text x={cx} y={cy + 8} textAnchor="middle" fill="#09DFAB" fontSize="13" fontFamily="system-ui,sans-serif" fontWeight="800" letterSpacing="1">FORMA</text>
          <text x={cx} y={cy + 24} textAnchor="middle" fill="rgba(9,223,171,0.55)" fontSize="10" fontFamily="system-ui,sans-serif">INTEGRADA</text>
        </motion.svg>
      </div>
    </div>
  )
