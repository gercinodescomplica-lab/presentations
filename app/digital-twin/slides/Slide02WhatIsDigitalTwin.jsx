...
export default function Slide02WhatIsDigitalTwin() {
  ...
  const blocks = t('slide02.blocks', { returnObjects: true }).map((b, i) => ({
    ...b,
    color: COLORS[i % COLORS.length],
  }))

  return (
    <SlideCard slideNum="02" label={t('slide02.label')}>
      <div className="flex h-full">
        <LeftPanel>
          ...
          >
            <TagLabel>{t('slide02.tag')}</TagLabel>
          </motion.div>
          ...
          >
            <SubtitleBar>{t('slide02.subtitle')}</SubtitleBar>
          </motion.div>
          ...
          >
            ...
            <span className="text-[#00E5FF] text-xs font-mono tracking-widest">{t('slide02.badge')}</span>
          </motion.div>
        </LeftPanel>
